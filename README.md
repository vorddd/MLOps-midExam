...
## CI/CD Workflow & Local Verification

Workflow `.github/workflows/ci-cd.yml` otomatis berjalan pada setiap *push* atau *pull request* ke branch `main`. Pipeline melakukan tiga hal utama:

1. Menyiapkan lingkungan Python 3.11 dan meng-*cache* dependensi `deployment/requirements.txt` serta `requirements-dev.txt`.
2. Menjalankan `pytest` sehingga integritas dataset dan artefak model terus terpantau.
3. Membangun image Docker Streamlit dan mengunggah file `streamlit-shipping.tar` sebagai artefak siap-deploy.

Untuk menjalankan pengecekan lokal sebelum mendorong kode:

```bash
python -m venv .venv
.venv\Scripts\activate  # Gunakan source .venv/bin/activate di Linux/Mac
pip install -r deployment/requirements.txt -r requirements-dev.txt
pytest
```

Membangun container Streamlit secara manual:
...
Dengan alur ini, setiap perubahan kode akan langsung tervalidasi dan menghasilkan artefak deployment yang konsisten.
